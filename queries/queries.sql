...
FROM persona p
LEFT JOIN profesor i ON p.id = i.id_profesor
LEFT JOIN departamento d ON i.id_departamento = d.id
...
WHERE EXISTS (
    ...
    INNER JOIN  grado g
		ON a.id_grado = g.id
	WHERE a.id_profesor = p.id_profesor
    AND LOWER(g.nombre) like '%ingeniería informática%');
...
FROM persona p
INNER JOIN alumno_se_matricula_asignatura m
	ON p.id = m.id_alumno
...
INSERT INTO persona(nif, nombre, apellido1, apellido2, ciudad, direccion, fecha_nacimiento,sexo,tipo)
VALUES ('Z4500128P','Edwin','Barrera','Ceron','Barcelona','Gran Via 69', '1990-11-28','M','profesor');

SELECT  d.nombre AS nombre_departamento, p.apellido1, p.apellido2, p.nombre
FROM persona p
LEFT JOIN profesor i ON p.id = i.id_profesor
LEFT JOIN departamento d ON i.id_departamento = d.id
WHERE p.tipo = 'profesor' -- AND i.id_departamento IS NULL
ORDER BY
...
LEFT JOIN profesor i ON p.id = i.id_profesor
LEFT JOIN departamento d ON i.id_departamento = d.id
WHERE p.tipo = 'profesor' AND i.id_departamento IS NOT NULL
GROUP BY d.nombre
ORDER BY
    total ASC
    ;
...
FROM persona p
LEFT JOIN profesor i ON p.id = i.id_profesor
RIGHT JOIN departamento d ON i.id_departamento = d.id 
GROUP BY d.nombre
ORDER BY
    total ASC
    ;
